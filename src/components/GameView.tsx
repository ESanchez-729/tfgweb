...
const GameView = () => {
    ...
    const [userLibraryReview, setUserLibraryReview] = useState<boolean>(false)
    const [selectedStatus, setSelectedStatus] = useState<StatusEnum>(StatusEnum.PLAN_TO_PLAY)
    const [resgistersWReviews, setRegistersWReviews] = useState<LibrarySB[]>([])
    ...
    }, [queryParams.get('id'), vm.loggedIn])
    ...
    const loadReviews = async () => {
        await vm.getDB
            ...
            .then((response) => {
                if (response.data !== undefined && response.data !== null) {
                    setRegistersWReviews(response.data)
                } else {
                    console.log(response.error)
                    setRegistersWReviews([])
                }
            })
    }
    ...
    const postReview = async (value: number, reviewText: string) => {
        const {data, error} =  await vm.getDB
          .from<LibrarySB>('library')
          .select()
          .eq('user_id', vm.getCurrentUserId || "")
          ...
          if(data !== null && data !== undefined) {
            ...
            data[0].score = value || 0
            data[0].review = reviewText || ""
                    ...
                    <Box sx={{ display: "flex", flexDirection: "row", mt: "3%", justifyContent: "flex-start", ml: "-0.5em" }}>
                        ...
                        {(userLibraryStatus !== "") ?
                            ...
                            : (vm.loggedIn) ? <Chip label="Add"
                                ...
                                : <Button sx={{
                                    fontWeight: "bold", backgroundColor: '#2196f3', ml: "-10%", color: "white", height: "2.5em", width: "10em", borderRadius: "16px", mt: "1.3em", fontSize: "80%",
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", minWidth: "31%" }}>
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                            ...
                            <Box sx={{ display: "flex", flexDirection: "row", justifyContent: "space-between", width: "100%" }}>
                                ...
                                {(vm.loggedIn && !userLibraryReview && userLibraryStatus !== "")?
                                    <NewReviewDialog createFunction={postReview} editRating={3} editText={""} buttonName={"Add review"} />
                                : null}
                            </Box>
                            <Box sx={{ display: "flex", flexDirection: "column" }}>
                                {resgistersWReviews.map((data) => {
                                    return (
                                        <ReviewCard deleteFunction={deleteReview} userID={data.user_id || ""} rating={data.score || 0} review={data.review || ""} updateFunction={postReview} />
                                    )
                                } )}
                            </Box>
                        </Box>
